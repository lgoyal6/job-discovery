...
A second digest runs out of the same code for a second reader. `JOB_PROFILE=finance` swaps four things at once: the source list (three finance lists, ten LinkedIn searches named after the roles that reader asked for, and 65 ATS boards rather than the 257 technical ones, with no Apify actors, watched program pages or applied ledger), the role rules, the recipient via `FINANCE_EMAIL_TO`, and the database via `FINANCE_DATABASE_URL`. Boards and LinkedIn queries carry the profile they belong to and are filtered as they load; a board marked `both`, which is every trading firm and investment manager on the list, is fetched by either digest. Its own database is not a preference: `jobs.sent_at` is one column and `email_batches` carries no recipient, so two profiles against one database would mean the second reader never sees a role the first was mailed, and `pipeline` refuses to start without it. With `JOB_PROFILE` unset the run is byte-identical to what it was before the profile existed.

Which source is fastest is a question the pipeline can answer about itself, and the answer decided what it pays for. Measured over its own history, on postings published within the last ten days, the median time from publication to the digest is 6 hours for the Simplify list, 12 for its off-season twin, 17 for vanshb03, 19 for the free LinkedIn guest endpoint, 20 for speedyapply and 29 for the paid Apify actors. When several sources carry one role, the free guest endpoint arrives first 62% of the time and the paid actors 13%. So the paid LinkedIn actor was buying a slower copy of something free, and is gone; Monster stays, on different ground, because it is the only source that returns the employer's own description, which is what the sponsorship rules read.

The employer's own board is not the fastest either, which is worth saying plainly: on roles both carry, the boards run 8 hours behind Simplify, 45 behind vanshb03 and 81 behind sndsh404, because those lists scrape boards continuously while this polls them every six hours. Boards earn their place on link quality and on covering employers no list tracks, not on latency.

Boards are found rather than guessed. Probing 234 unconfigured watchlist companies by slug returned nothing at all, because what is left on that list is Google, Meta, Amazon, Apple and Microsoft, none of which use Greenhouse, Lever or Ashby. Reading the apply URLs already in the database found 119: a slug cannot be guessed from a company name, but it is written verbatim in the link every list publishes.
